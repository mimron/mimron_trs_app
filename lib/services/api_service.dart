...
class ApiService {
  ...
  Future<Response> getMemberList() async {
    var memberUrl = Uri.parse('${Constants.BASE_URL}/members');
    final res = await client.get(memberUrl);
    return res;
  }

  Future<Response> getMemberById(String id) async {
    var memberUrl = Uri.parse('${Constants.BASE_URL}/members/$id');
    final res = await client.get(memberUrl);
    return res;
  }

  Future<Response> addMember(String branchId, String repId, String name,
      String currentPosition, String managerId) async {
    var memberUrl = Uri.parse('${Constants.BASE_URL}/members');
    final res = await client.post(memberUrl, body: {
      "branch_id": branchId,
      "rep_id": repId,
      "name": name,
  ...
  Future<Response> updateMember(int? id, String branchId, String repId,
      String name, String currentPosition, String managerId) async {
    var memberUrl = Uri.parse('${Constants.BASE_URL}/members/$id');
    final res = await client.put(memberUrl, body: {
      "branch_id": branchId,
      "rep_id": repId,
      "name": name,
      "current_position": currentPosition,
  ...
  Future<Response> deleteMember(String id) async {
    var memberUrl = Uri.parse('${Constants.BASE_URL}/members/$id');
    final res = await client.delete(memberUrl);
    return res;
  }
}
